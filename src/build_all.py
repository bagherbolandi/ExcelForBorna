...
def main():
    ...
    print("\n[1/4] Building the workbook model (53 sheets, real formulas)...")
    ...
    print("\n[3/4] Executing 20 mandatory test cases...")
    results = run_all()
    ...
    print("\n[4/4] Rendering to .xlsx (RTL + gates + protection + protected ranges)...")
    render(model, out)
    size = os.path.getsize(out)
    print(f"   wrote {out} ({size:,} bytes)")

    # Also write a CSV snapshot of the 20 tests for the docs
...
def _write_test_csv(results, path):
    ...
    with open(path, "w", newline="", encoding="utf-8") as f:
        ...
        for tid, d in results:
            w.writerow([tid, d["input"], d["expected"], d["actual"], d["pass_fail"], d["issue"], "2026-09-23", "USR-09"])
    print(f"   wrote {path}")
