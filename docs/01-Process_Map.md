...
## 1.6 نسخه 2.0 — نقشه ۹ مرحله‌ای کامل (سفارش تا تصویب مدیریت ارشد)

نسخه 2.0 چرخه را به ۹ مرحله با مالکیت صریح واحدها و گیت‌های سخت گسترش می‌دهد
(جزئیات فنی گیت‌ها: سند 11):

```
[فروش]            ۱. دریافت و بررسی سفارش ── READY ──►
[مدیر پروژه]      ۲. تعریف پروژه + تهیه گانت + تکمیل امکان‌سنجی ── امکان‌سنجی کامل ──►
[مهندسی]          ۳. تهیه اقلام (BOM) + تجهیزات + زمان‌سنجی + نفرساعت ── تأیید مهندسی/BOM/زمان‌سنجی ──►
[بازرگانی]        ۴. تعیین قیمت اقلام و تجهیزات (نرخ ارز برای وارداتی) ── همه ردیف‌ها قیمت‌دار ──►
[برنامه‌ریزی+انبار] ۵. مقدار مورد نیاز اقلام برای تولید + زمان‌بندی نیاز + رسید انبار ── وضعیت تأمین کامل ──►
[مالی]            ۶. قیمت تمام‌شده محصول + بودجه راه‌اندازی ── تأیید Costing ──►
[فروش]            ۷. قیمت نهایی فروش + نحوه پرداخت ── پیشنهاد Submitted ──►
[مدیریت پروژه]    ۸. جمع‌بندی عملکرد واحدها + به‌روزرسانی گانت/امکان‌سنجی + اعلام نتیجه به ارشد ── گزارش ارسال شد ──►
[مدیریت ارشد]     ۹. تأیید / رد / برگشت برای اصلاح
```

### شیت‌های افزوده در نسخه 2.0
| شیت | نقش | مالک ویرایش |
|---|---|---|
| `Feasibility` | امکان‌سنجی پنج‌بُعدی (بازار/فنی/اقتصادی/زمان‌بندی/ریسک) با وزن‌های قابل تنظیم از Settings | مدیر پروژه |
| `Equipment` | **لیست مرجع تجهیزات** | مهندسی |
| `Project_Equipment` | انتخاب تجهیزات پروژه از مرجع (مهندسی) + قیمت‌دهی با نرخ ارز (بازرگانی) | مهندسی/بازرگانی |
| `Time_Study` | زمان‌سنجی عملیات و نفرساعت → مصرف مستقیم در دستمزدِ قیمت تمام‌شده | مهندسی |
| `Receipts` | رسیدهای انبار متصل به ردیف‌های برنامه‌ریزی | انبار |
| `PMO_Summary` | جمع‌بندی عملکرد واحدها + اعلام به ارشد | مدیر پروژه |
| `Gate_Status` | پرچم‌های تکمیل ۸ مرحله + گیت‌های تجمعی + مرحله جاری | سیستم (فرمول) |
| `Gantt` | نمودار گانت فعالیت‌ها (میله‌های هفتگی، رنگ وضعیت) | مدیر پروژه (از طریق Schedule) |
| `Access_Control` | نقش‌ها، مالکیت شیت‌ها، گذرواژه‌های محدوده ویرایش | راهبر |

### به‌روزرسانی‌های نسخه 2.0 در شیت‌های موجود
- `Planning`: ستون‌های `Need_Date` (زمان‌بندی نیاز به اقلام) و `Suggested_Order_Date` (= تاریخ نیاز − لیدتایم).
- `Cost_Lines`: دستمزد مستقیم به‌جای عدد ثابت، **زنده** از جمع نفرساعت `Time_Study` × نرخ دستمزد `Settings`.
- `Dashboard`: ماتریس گردش کار پروژه‌ها (✅ تکمیل / 🔵 در جریان / 🔒 قفل) از `Gate_Status` + دو نمودار.
- `Workflow`: بازنویسی به نقشه ۹ مرحله‌ای با گیت‌ها و مالکان.
- `Error_Checks`: کدهای جدید `E-022..E-026` برای پایش گیت‌ها.
